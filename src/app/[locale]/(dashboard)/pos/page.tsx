...
import { Search, Plus, Minus, Trash2, CreditCard, Banknote, X, Beef, Scissors, Ribbon, Circle, Pill, CookingPot, BedDouble, Pipette } from "lucide-react";
...
const DEMO_PRODUCTS = [
  { id: "1", name: "Premium Dog Food 3kg", sku: "DOG-001", price: 250, icon: Beef, stock: 15 },
  { id: "2", name: "Cat Grooming Kit", sku: "CAT-002", price: 180, icon: Scissors, stock: 8 },
  { id: "3", name: "Leather Pet Collar", sku: "ACC-003", price: 120, icon: Ribbon, stock: 22 },
  { id: "4", name: "Interactive Ball Toy", sku: "TOY-004", price: 75, icon: Circle, stock: 30 },
  { id: "5", name: "Vitamin Supplements", sku: "HLT-005", price: 95, icon: Pill, stock: 12 },
  { id: "6", name: "Steel Bowl Set", sku: "ACC-006", price: 85, icon: CookingPot, stock: 18 },
  { id: "7", name: "Pet Bed Large", sku: "BED-007", price: 450, icon: BedDouble, stock: 5 },
  { id: "8", name: "Shampoo 500ml", sku: "GRM-008", price: 65, icon: Pipette, stock: 25 },
];
...
export default function POSPage() {
  ...
  function addToCart(product: (typeof DEMO_PRODUCTS)[0]) {
    setCart((prev) => {
      ...
      return [...prev, { id: product.id, name: product.name, price: product.price, quantity: 1, icon: product.icon }];
    });
  }
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 overflow-y-auto">
          {filtered.map((product) => (
            ...
            >
              <div className="w-8 h-8 mb-1 text-paws-orange"><product.icon className="w-8 h-8" /></div>
          ...
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div key={item.id} className="flex items-center gap-2 bg-neutral-50 rounded-xl p-2">
                <div className="w-6 h-6 text-paws-orange shrink-0"><item.icon className="w-6 h-6" /></div>
